...
genotypes <- readStandardGenotypes(N=100, filename='genotypes_hapgen.controls.caspase',
                                    format="plink")


causalSNPs <-getCausalSNPs(N=100, genotypes = genotypes$genotypes,NrCausalSNPs = 15, verbose = FALSE)
genFixed <-geneticFixedEffects(N = 100, P = 15, X_causal = causalSNPs)

genotypes <- readStandardGenotypes(N=100, filename='genotypes_subset_hapgen.controls',
                                   format="plink")
...
write.table(Y, 'Y_caspase', sep = '\t', col.names = FALSE)
